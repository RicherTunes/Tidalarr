...
public class TidalApiClientInvalidJsonMoreTests
{
    ...
    public async Task GetAlbumAsync_InvalidJson_ThrowsJsonException()
    {
        ...
        await Assert.ThrowsAsync<System.Text.Json.JsonException>(() => client.GetAlbumAsync("al1"));
    }
    ...
    {
        var client = new TidalApiClient(new HttpClient(new BadJsonHandler()), new Auth());
        await Assert.ThrowsAsync<System.Text.Json.JsonException>(() => client.GetAlbumTracksAsync("al1"));
    }
    ...
    {
        var client = new TidalApiClient(new HttpClient(new BadJsonHandler()), new Auth());
        await Assert.ThrowsAsync<System.Text.Json.JsonException>(() => client.SearchAsync("abc"));
    }
}
