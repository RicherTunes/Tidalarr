...
public class TidalApiClientNullTrackTests
{
    ...
    [Fact]
    public async Task GetTrackAsync_NullJson_ThrowsInvalidOperation()
    {
        var api = new TidalApiClient(new HttpClient(new NullHandler()), new Auth());
        await Assert.ThrowsAsync<InvalidOperationException>(() => api.GetTrackAsync("t1"));
    }
}
