...
2. Copy `scripts/ci.ps1`, `scripts/verify-local.ps1`, and the shared lint workflow pattern from `.github/workflows/ci.yml` into the target repo.
...
Every plugin should run the shared Common lint/parity gates in `.github/workflows/ci.yml`. Build/package verification should run through `scripts/ci.ps1` locally or in a dedicated build workflow when host assemblies and runner resources are available. Together these checks ensure:
