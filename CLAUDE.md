...
| `PerformanceMonitor` double-registered | LOW | 2026-07-16 | `RegisterSharedLibraryServices` and `ConfigureServices` each called `AddSingleton<PerformanceMonitor>()` — collapsed to the single `RegisterSharedLibraryServices` registration. Pinned by `TidalModuleDiTests.PerformanceMonitor_IsRegisteredExactlyOnce`. (The `TidalResponseCache` half of this entry was already fixed by T-3 via `TidalResponseCacheFactory`.) |
| Download tracker items never set `TotalSize` | LOW | 2026-07-16 | Lidarr's queue showed `0/0` for every Tidal download because tracker items never populated `TotalSize`. `TidalLidarrDownloadClient.Download`'s item factory (extracted as the `CreateTrackedDownloadItem` seam) now seeds `TotalSize` from the grabbed release's per-quality size estimate (`ReleaseInfo.Size`, computed by the indexer via Common's `AlbumSizeEstimator`); `ProjectDownloadItems` derives `RemainingSize` from progress. Pinned by `TidalLidarrDownloadClientTotalSizeTests`. |
| D-12: three runtime caches over one credential set | MEDIUM | 2026-07-16 | Indexer, download client, and favorites import list each had their own `HostBridgeRuntimeCache` subclass building a private `ServiceProvider` — three singleton `TidalOAuthService` instances racing over one rotating-refresh-token file (the T-2 clobber class; the in-service mitigation `RefreshOrClearOnRevokedAsync` remains as defense-in-depth). Collapsed into ONE `TidalRuntimeCache` keyed on ConfigPath (the token file's home) with a shared `TidalRuntime` (lazy download orchestrator; per-side settings applied in place — RedirectUrl is a one-time OAuth input, deliberately NOT part of the key). All three host classes now resolve the SAME `TidalOAuthService`, so refresh single-flight actually covers them. Pinned by `TidalRuntimeCacheTests` (share/join/distinct-paths/RedirectUrl-reuse/sub-state/disposal). |
