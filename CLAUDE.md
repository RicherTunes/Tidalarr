...
- `LrclibClient` (via `LyricsEnricher`) — `src/Tidalarr/Application/Services/LyricsEnricher.cs` + `src/Tidalarr/Integration/TidalDownloadClient.cs` (step 7). Wave-31 adoption: best-effort synced-lyrics (.lrc) fetch alongside audio downloads via LRCLIB public API.
